#!/usr/bin/env bash
# land-the-paper-reaches-its-page.sh  ·  run from the REPO ROOT
#
...
git log --oneline -20 | grep -q 'prints on Tuesday' || die "the Tuesday commit is not here - this fixes it, land it first"
git log --oneline -20 | grep -q 'paper reaches its page' && die "already landed - re-running is a no-op by design"
