#!/usr/bin/env bash
# land-a-reply-carries-its-thread-2.sh  ·  run from the REPO ROOT
#
# A reply quotes what it is answering.
#
# land-the-rail-belongs-to-the-room.sh DID run — its two fixes are in as
# 430e95ce, and this script no longer claims them. Sidebar.tsx and hubs.ts are
# left out of the commit entirely; what remains is the quotation and the reply
# link that carries its room.
#
# A REPLY CARRIES ITS THREAD. Replies left this city with nothing under
...
ALLOWED_IN='^(M |MM| M|\?\?) together-city-react/src/(index\.css|app/a-read-section-folds-itself\.test\.ts|features/mail/(replyQuote\.ts|replyQuote\.test\.ts|pages/(Compose|MessageView)\.tsx))$'
...
git add $W/src/features/mail/replyQuote.ts \
        ...
        $W/src/index.css \
        land-a-reply-carries-its-thread-2.sh
...
THE REPLY KEEPS THE ROOM IT WAS READ IN. A reply opened from a filed
conversation now carries `?project=<key>` in its link. The SEND already
inherited the project - the thread is filed, and inheritance is rule one - but
the RAIL did not, and a composer whose sidebar says All Emails while it writes
into ABG is the same lie the rail commits went to remove.
