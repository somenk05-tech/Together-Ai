#!/usr/bin/env bash
# land-the-profile-prints-on-tuesday-2.sh  ·  run from the REPO ROOT
#
# ── WHY -2 ───────────────────────────────────────────────────────────────────
# The patch is unchanged. The original's scope loop walked EVERY git-status
# entry, untracked scratch included — and the repo root carries six-day-old
# strays (astrology-night-surface.patch, button-preview.html, …) that sat
# hidden behind the spend-log's modified files, which git lists first. The
# moment the money patch landed, the loop started dying on scratch from 7 Aug
# that no session is working on (mtimes checked before writing this).
#
# So the scope check takes the money script's shape instead: STRICT inside the
# two packages — anything there, tracked or untracked, aborts — and tolerant of
# untracked scratch outside them, while a tracked file modified outside the
# packages still aborts. The original's name had been run, so per the freezing
# rule this ships as -2 rather than an edit in place.
set -uo pipefail
...
ALLOWED_IN='^(M |MM| M) together-city-react/src/(styles/(tokens|relief)\.css|features/nutrition/pages/Preferences\.tsx)$'
...
(Shipped by the -2 script: the original's scope loop died on six-day-old
untracked scratch at the repo root once the spend-log patch stopped hiding it.
The patch is byte-identical; only the scope check changed shape.)
MSG
