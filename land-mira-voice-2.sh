#!/usr/bin/env bash
# land-mira-voice-2.sh — the 70/15 ratio, made into something a test can fail on.
#
#   cd ~/Documents/GitHub/Together-Ai
#   bash land-mira-voice-2.sh && git push
#
...
RAW=$(grep -nE "startBy" "$API/src/mira/mira.service.ts" | grep -v "clockTime(" || true)
if [ -n "$RAW" ]; then
  printf '%s\n' "$RAW"
...
if grep -nE "startBy" "$SCRATCH" | grep -v "clockTime(" >/dev/null; then
  rm -f "$SCRATCH"
else
...
grep -nE 'wants starting|\$\{what\} (?:wants|is next)\.' "$API/src/mira/mira.service.ts" \
