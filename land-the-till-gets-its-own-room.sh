#!/usr/bin/env bash
# land-the-till-gets-its-own-room.sh  ·  run from the REPO ROOT
#
...
say "2 - scope"
STRAY="$(git status --porcelain -- "$A/src/fitness/supplements" "$W/src/features/fitness" "$W/src/api/store.api.ts" "$W/src/app/router.tsx" "$W/src/config/hubs.ts" \
...
say "6 - commit"
git add "${FILES[@]}" land-the-till-gets-its-own-room.sh || die "git add"
