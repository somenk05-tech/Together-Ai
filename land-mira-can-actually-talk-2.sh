#!/usr/bin/env bash
# land-mira-can-actually-talk-2.sh  ·  run from the REPO ROOT
#
# -2 because the first run aborted on its own scope guard: git status reports
# an untracked DIRECTORY as "?? path/" — the migration folder, with a
# trailing slash — and the filter only knew the file inside it. The frozen
# rule applies to the script that has been run, so the fix is this name.
#
...
check "$A/src/mira/she-can-actually-talk.spec.ts" 5a9125fbad4d2af16a1ea8d3cbef821ad1768bea95bb3736b3c9d7651da44eec
