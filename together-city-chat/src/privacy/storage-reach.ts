...
export const STORAGE_COLUMNS: StorageColumn[] = [
  ...
  { model: 'MedicalEmailAttachment', column: 'storageKey', holds: 'private-object', carriedAwayBy: 'MedicalMailService.remove(forever) + emptyTrash for an unfiled file, MedicalService.deleteRecord for a filed one (which nulls this column), and purge rule storageKey', reason: 'A file that arrived by Medical Mail, in the health vault. Once filed it is also a MedicalRecord.fileKey; before that, this column is the only name the object has.' },
  ...
  { model: 'PostMedia', column: 'hlsUrl', holds: 'private-object', carriedAwayBy: 'the same two paths as PostMedia.url — SocialService.deletePost via storageKeys(), and AuthService.purgePostObjects at account deletion — both of which expand it through ladderKeysOf() rather than deleting the one key it holds', reason: 'The master playlist of the adaptive ladder (6 Sep), and the ONE column here that names more than one object: nine keys live under its prefix and the row stores only the pointer. That is why both deletion paths run it through ladderKeysOf() — naming just this key would leave every rung of the video in the bucket, which is the exact shape of the five leaks this registry was built after.' },
  ...
  { model: 'CsamHit', column: 'storageKey', holds: 'private-object', carriedAwayBy: 'NOTHING, DELIBERATELY — this is the one file in the city that is kept on purpose. media/hash-match refuses without its usual delete step, the three guards were changed to return that refusal without calling refuse(), and the purge plan keeps the row. Recorded here as a decision rather than an absence, in the same spirit as the Attachment entry below', reason: 'A pointer to material that matched a known-bad hash list. `bucket` on the same row says which of the two it is in, because a hit can arrive from a dating photo (private), a snap (private), a chat image (public) or a post (private). Destroying it would destroy the report with it.' },
  ...
  { model: 'JobProfile', column: 'resumeUrl', holds: 'private-object', carriedAwayBy: 'JobsService.deleteResume (30 Aug; both shapes since 2 Sep) + purge rule storageKeys AND storageUrls', reason: 'The uploaded CV document. A vault key under cv/<userId>/ since 2 Sep (launch blocker 3); rows from before hold a public URL until scripts/move-private-files-to-vault.ts has run, which is why the purge rule names the column under both clauses.' },
  ...
  { model: 'ServiceVerification', column: 'docUrl', holds: 'public-object', carriedAwayBy: 'LocalServicesService.purgeListingObjects + the listing’s purge rule', reason: 'THE MOST SENSITIVE OF THESE. A document an owner submitted to prove the business is real. It cascaded away with the listing and stayed in a public bucket indefinitely. Since 2 Sep the DTO no longer accepts it and the service writes null; the column stays for rows written before, and the purge still reads it.' },
  ...
  { model: 'BeautyOrder', column: 'addressJson', holds: 'not-storage', reason: 'The delivery address the citizen typed at checkout, as JSON in the row — a street and a pin, not a file. Purged with the order under the purge plan.' },
  { model: 'SupplementOrder', column: 'addressJson', holds: 'not-storage', reason: 'The same delivery address, on the Fitness store’s order row — JSON in the row, not a file.' },
  ...
  { model: 'MedicalEmailAttachment', column: 'filename', holds: 'not-storage', reason: 'The name the sender gave the file, for the row on the page. The object is storageKey above.' },
  { model: 'MedicalEmailAttachment', column: 'documentType', holds: 'not-storage', reason: 'The vault folder the reader chose (record-reader RECORD_KINDS) — a word, not a key.' },
