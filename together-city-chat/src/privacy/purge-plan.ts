...
export const PURGE_RULES: PurgeRule[] = [
  ...
  { model: 'ProgrammeAddition', by: 'userId', action: 'purge', reason: 'The movements they added to days of their own month. Theirs alone; nobody else can see them.' },
...
/* PROPERTY WAS NOT CLASSIFIED AT ALL, AND THE GUARD COULD NOT SAY SO.
  ...
  { model: 'JobProfile', by: 'userId', action: 'purge', storageKeys: ['resumeUrl'], storageUrls: ['resumeUrl', 'photoUrl'], reason: 'Their CV — history, skills, salary expectations — AND the uploaded document and photograph themselves. Both live in the public bucket as URLs, so this rule was `purge` with no storage clause and read as complete while leaving a stranger\u2019s CV addressable forever. Since 2 Sep the CV is a vault key under cv/<userId>/ and resumeUrl is named under BOTH clauses: storageKeys for the key it holds now, storageUrls for the public URL it held before the migration ran — each clause ignores the other shape.' },
