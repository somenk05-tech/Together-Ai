...
const product = (over: Partial<RecommendedProduct> & { id: string; category: string; usage: string }): RecommendedProduct => ({
  name: over.id, brand: 'Together Beauty Labs', priceInr: 900, tags: [], profileKeys: [],
  suitableSkin: ['all'], actives: [], blurb: '', keyIngredient: '', matched: true,
  ...over,
...
describe('which routine a product lands in', () => {
  ...
  it('sends a Weekly product to the weekly routine only', () => {
    const r = buildRoutines([product({ id: 'mask', category: 'Haircare', usage: 'Weekly' })]);
    expect(weeklyOf(r).steps).toHaveLength(1);
    expect(morningOf(r).steps).toHaveLength(0);
    expect(weeklyOf(r).steps[0].frequency).toBe('Once a week');
  });
  ...
  it('always returns all three routines, even when empty', () => {
    const r = buildRoutines([]);
    expect(r.map((x) => x.timeOfDay)).toEqual(['morning', 'evening', 'weekly']);
