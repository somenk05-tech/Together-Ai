...
describe('registration', () => {
  ...
  it('creates a fully-initialised, verification-pending account', async () => {
    const { f, svc } = build();
    const res = await svc.register({ handle: 'John', name: 'John', email: 'John@Mail.com', password: STRONG, dateOfBirth: '1995-06-15', gender: 'female' } as never);
  ...
  it('rejects a weak password', async () => {
    const { svc } = build();
    await expect(svc.register({ handle: 'jane', name: 'Jane', email: 'j@e.com', password: 'weak', dateOfBirth: '1995-06-15', gender: 'female' } as never)).rejects.toThrow();
  });
  it('rejects a duplicate handle', async () => {
    const { svc } = build();
    await svc.register({ handle: 'dup', name: 'A', email: 'a@e.com', password: STRONG, dateOfBirth: '1995-06-15', gender: 'female' } as never);
    await expect(svc.register({ handle: 'dup', name: 'B', email: 'b@e.com', password: STRONG, dateOfBirth: '1995-06-15', gender: 'female' } as never)).rejects.toThrow(/handle/i);
  });
  it('rejects a duplicate email', async () => {
    const { svc } = build();
    await svc.register({ handle: 'a1', name: 'A', email: 'same@e.com', password: STRONG, dateOfBirth: '1995-06-15', gender: 'female' } as never);
    await expect(svc.register({ handle: 'a2', name: 'B', email: 'same@e.com', password: STRONG, dateOfBirth: '1995-06-15', gender: 'female' } as never)).rejects.toThrow(/email/i);
  });
