...
describe('registration', () => {
  ...
  it('creates a fully-initialised, verification-pending account', async () => {
    const { f, svc } = build();
    const res = await svc.register({ handle: 'John', name: 'John', email: 'John@Mail.com', password: STRONG, dateOfBirth: '1995-06-15', gender: 'female', orientation: 'preferNotToSay' } as never);
  ...
  it('rejects a weak password', async () => {
    const { svc } = build();
    await expect(svc.register({ handle: 'jane', name: 'Jane', email: 'j@e.com', password: 'weak', dateOfBirth: '1995-06-15', gender: 'female', orientation: 'preferNotToSay' } as never)).rejects.toThrow();
  });
