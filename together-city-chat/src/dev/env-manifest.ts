...
export const ENV_MANIFEST: EnvEntry[] = [
  ...
  { name: 'TURN_SHARED_SECRET', group: 'Messaging & calls', purpose: 'TURN REST API secret (coturn use-auth-secret, or any provider that supports it): each citizen gets a relay credential of their own that expires (5 Sep).', whenMissing: 'If METERED_APP + METERED_API_KEY are set, those mint the credential; otherwise the static pair in ICE_SERVERS is handed to every account — anybody who has seen it can relay on the city\'s bill.', secret: true },
  { name: 'METERED_APP', group: 'Messaging & calls', purpose: 'The metered.ca app subdomain (<app>.metered.live) whose credentials endpoint mints a per-citizen, expiring TURN credential.', whenMissing: 'See TURN_SHARED_SECRET — one of the two minting routes.' },
  { name: 'METERED_API_KEY', group: 'Messaging & calls', purpose: 'The metered.ca API key for that endpoint.', whenMissing: 'See TURN_SHARED_SECRET.', secret: true },
