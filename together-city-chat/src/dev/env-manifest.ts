...
export const ENV_MANIFEST: EnvEntry[] = [
  ...
  { name: 'MEDIA_CDN_BASE', group: 'Media', purpose: 'Where the media-edge Worker answers, e.g. https://media.togethercity.tech. Post media is then served from Cloudflare\u2019s cache for a token this API signs (workers/media-edge). Set the Worker\u2019s LINK_SECRET to the same value as JWT_ACCESS_SECRET \u2014 both derive the media key from it.', whenMissing: 'Post media keeps the presigned R2 links it has today. Those only work on the r2.cloudflarestorage.com S3 endpoint, which Cloudflare does NOT cache \u2014 so every photograph in every feed page is fetched from storage on every request, with no edge in between.' },
