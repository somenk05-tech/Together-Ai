...
export const ENV_MANIFEST: EnvEntry[] = [
  ...
  { name: 'STRICT_PROD_CONFIG', group: 'Core', purpose: 'Used to refuse to boot on any production-config warning. Since 8 Sep it only makes the boot banner louder \u2014 a warning must never be an outage; the refusals that remain are the fixed fatal list (JWT secrets, the private vault, the attachment origin).', whenMissing: 'Correct. Leave it unset; the warnings print either way and are listed here.', expectUnset: true },
  ...
  { name: 'FFMPEG_PATH', group: 'Media', purpose: 'Where ffmpeg is, for the video worker that re-encodes a post\u2019s video into H.264 MP4 the whole city can play (5 Sep). Unset, the binary ffmpeg-static ships is used, then ffmpeg on PATH.', whenMissing: 'Nothing, on a build where ffmpeg-static installed. Without any ffmpeg at all, every posted video is marked failed and plays nowhere but the poster.' },
  ...
  { name: 'ANTHROPIC_SKIN_MODEL', group: 'AI', purpose: 'The model that reads a face for the beauty analysis. Separate from the vision model since 6 Sep and cheaper by default: the whole output is a thirteen-value enum, a short note and up to four boxes, and on the free tier this is the largest vision line in the city.', whenMissing: 'Uses the built-in default (the Sonnet tier). Set it back to the vision model to disagree, with one variable rather than a deploy.' },
  ...
  { name: 'CSAM_MATCH_URL', group: 'Safety', purpose: 'An endpoint speaking the known-bad-hash contract in media/hash-match/hash-match.provider.ts — one POST, JSON in, JSON out — in front of whichever service this deployment is approved for (Cloudflare\u2019s CSAM Scanning Tool, Thorn Safer, PhotoDNA). It runs BEFORE Rekognition on every image surface, because label detection answers "does this look explicit" and only a hash match answers "is this a known image".', whenMissing: 'THE GATE FAILS CLOSED: no photograph can be posted, sent or approved anywhere in the city. Deliberate \u2014 a CSAM gate that quietly approves when it is unconfigured is worse than none, because it is believed. Until a provider is signed the word "off" bypasses the gate on purpose and is listed as a problem on every boot.' },
  { name: 'CSAM_MATCH_KIND', group: 'Safety', purpose: 'Which protocol lives at CSAM_MATCH_URL. Unset is the JSON-in, JSON-out contract in media/hash-match/hash-match.provider.ts. \u2018arachnid\u2019 is the Arachnid Shield API of the Canadian Centre for Child Protection \u2014 raw image bytes, Basic auth, an answer in classifications \u2014 which is free to electronic service providers and is the only known-bad-hash service reachable without a signed contract or an approved application.', whenMissing: 'The contract dialect is used, with CSAM_MATCH_TOKEN as a bearer token.' },
  { name: 'CSAM_MATCH_USER', group: 'Safety', purpose: 'Basic-auth username for a dialect that authenticates that way (Arachnid Shield). Register at projectarachnid.com.', whenMissing: 'With CSAM_MATCH_KIND=arachnid the matcher cannot authenticate, so the gate FAILS CLOSED and is listed as a problem at boot. It never falls back to the other dialect \u2014 Basic credentials sent as a bearer token is a 401 on every photograph wearing the badge of a configured matcher.' },
  { name: 'CSAM_MATCH_PASSWORD', group: 'Safety', purpose: 'Basic-auth password sent with CSAM_MATCH_USER.', whenMissing: 'Same as CSAM_MATCH_USER: the gate fails closed and says so.', secret: true },
  { name: 'CSAM_MATCH_TOKEN', group: 'Safety', purpose: 'Bearer token sent to CSAM_MATCH_URL.', whenMissing: 'The request is made without an Authorization header, which is correct for an adapter that authenticates another way.', secret: true },
  { name: 'CSAM_MATCH_TIMEOUT_MS', group: 'Safety', purpose: 'How long the matcher has to answer before the call is treated as an outage. Default 8000.', whenMissing: 'Eight seconds. A timeout is "unavailable", never a pass.' },
  ...
  { name: 'JOBS_CONCURRENCY', group: 'Operations', purpose: 'How many jobs of the `city` lane one container runs at once. Default 8.', whenMissing: 'Uses the built-in eight per container.' },
  { name: 'JOBS_MEDIA_CONCURRENCY', group: 'Operations', purpose: 'How many jobs of the `media` lane — video encodes and the stuck-media sweep — one container runs at once. Default 1: a 2 GB input plus its rendition is ~4 GB of ephemeral disk, and two encodes on one box is two at half speed. Scale with replicas on the worker service instead.', whenMissing: 'One encode at a time per container, which is the number to want.' },
  { name: 'WS_MAX_SOCKETS_PER_CITIZEN', group: 'Safety', purpose: 'How many live WebSocket connections one account may hold. Every rate limit on the gateway hangs off the socket object, so without this one account opening five hundred sockets had five hundred times every ceiling — and none of it passes the HTTP throttler, because a handshake is not a route. Default 6: a phone, a laptop and a couple of tabs.', whenMissing: 'Six. Counted on the Redis set that already tracks presence, so it is true across containers; with Redis away it degrades to per-instance along with everything else.' },
  { name: 'WS_SENDS_PER_CITIZEN', group: 'Safety', purpose: 'Messages one CITIZEN may send over sockets in a minute, across every connection and every container. The per-socket bucket (60) is unchanged and stays synchronous; this is the one that survives a reconnect. Default 180.', whenMissing: 'One hundred and eighty. Fails open when Redis is away, where the per-socket bucket and the connection cap are still in force.' },
  { name: 'JOBS_ROLE', group: 'Operations', purpose: '"api" produces jobs and runs no workers — what the containers serving citizens should be, so an hour of ffmpeg never lands on the box carrying the websockets. "worker" produces and consumes. "both" is the old behaviour.', whenMissing: 'Both, so a single-container deployment and every dev machine work with nothing set. In a two-service deployment this is what keeps the encode off the API — see docs/WORKER-SERVICE.md.' },
