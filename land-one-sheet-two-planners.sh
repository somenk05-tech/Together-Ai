#!/bin/bash
# land-one-sheet-two-planners.sh — the readability pass on the two sheets,
...
ALLOWED='^((M | M|MM) together-city-react/src/(styles/(tokens|relief)\.css|features/(nutrition/(dayBalance\.ts|pages/MealPlan\.tsx)|family/pages/Weekly\.tsx)))$|^\?\? together-city-react/src/features/nutrition/components/PressDay\.tsx$'
