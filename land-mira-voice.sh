#!/usr/bin/env bash
# land-mira-voice.sh — the 70/15 ratio, made into something a test can fail on.
#
#   cd ~/Documents/GitHub/Together-Ai
#   bash land-mira-voice.sh && git push
#
...
c2616eead4261b3090e92a48eff11d25fbe0277871026daa926fa6e121e2d2f7  together-city-chat/src/mira/mira.service.ts
...
# No machine strings in anything she says.
say "no raw hub field is interpolated into a sentence"
grep -nE '\$\{when\}|\$\{what\} wants' "$API/src/mira/mira.service.ts" \
  && die "a hub field is being conjugated again — quote it, do not conjugate it" || true
say "  none"
