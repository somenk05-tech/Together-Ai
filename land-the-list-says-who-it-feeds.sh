#!/bin/bash
# land-the-list-says-who-it-feeds.sh  ·  run from the REPO ROOT
#
# SUPERSEDES land-the-basket-knows-its-plan.sh, which was written this
# afternoon and never run: the owner then asked for the sheet redesign and the
# people count, both of which edit the same three API files, so that script's
# hashes went stale before its first run. Its patch is inside this one,
# whole and unchanged in meaning. Do not run the old script; it will refuse
# anyway (hash mismatch), and that refusal is correct.
#
# THREE THINGS, ONE STORY — the grocery list now says what it is FOR:
#   1. WHICH MENU. Locks remember the plan model that was showing (My
#      Preferences vs Optimal Health); the basket shops each locked day in the
#      model it was locked in; the panel and the chips say so in words.
#   2. HOW MANY PEOPLE. An individual list gains "Cooking for N" (1–12,
...
#   3. ON WHAT PAPER. The list prints as the owner's reference sheet — the
...
case "$LOG" in
  *"The basket knows its plan"*)
    echo "!! 'The basket knows its plan' already landed separately — this script's"
    echo "   commit message would claim work that commit did. Ask for a fresh script."
    exit 1 ;;
esac
case "$LOG" in
...
git add together-city-chat/src/nutrition/nutrition.service.ts \
        ...
        land-the-list-says-who-it-feeds.sh

git commit -F - <<'MSG'
The list says who it feeds

Three answers the grocery list owed and could not give: which MENU it is
buying for, how many PEOPLE it is buying for, and — the owner's reference art —
what it looks like as a thing you would actually carry into a shop.

── WHICH MENU ─────────────────────────────────────────────────────────────────

Every day has two real menus — My Preferences and Optimal Health — and the
basket only knew how to shop one: locks were bare day numbers and groceryPlan
composed 'preferred' HARDCODED, so accepting the Optimal Friday bought the
Preferences Friday's food. The lock now records the model that was showing
(owner's call, 13 Aug: PER DAY, not one followed plan — locking Friday from
Optimal must not silently re-shop a Tuesday accepted under Preferences).
extras carry composedLockModes beside composedLocks; absent means 'preferred',
because every lock predating the field was made when that was the only plan
the basket shopped. groceryPlan splits locked days by model, composes each
set in its own plan, merges one basket. The Shopping-for chips name each
locked menu's plan, the paragraph states the source in words, and a locked
day read from the other planner tab says which menu the lock means.
...
day-lock.spec grew from 12 to 21: the model is recorded, remembered,
forgotten on unlock, sanitised, and shopped per day; the people count scales,
persists, reads back, clamps, and is ignored by family on purpose.

(Supersedes the unrun land-the-basket-knows-its-plan.sh, whose hashes went
stale when the sheet work touched the same files.)
MSG
...
===============================================================
 ...
 "Cooking for N" on it, and lock a day from the Optimal tab
 to see the chips name their plan.
===============================================================
