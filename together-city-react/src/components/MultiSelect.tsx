  ...
  return (
    <div ref={rootRef} style={{ position: 'relative' }}>
      <div onClick={() => setOpen(true)} style={{
        ...
        {values.map((v) => (
          <span key={v} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'var(--accent)', color: '#fff', borderRadius: 999, padding: '4px 6px 4px 11px', fontSize: 12.5, fontWeight: 600 }}>
            {v}
            <button type="button" aria-label={`Remove ${v}`} onClick={(e) => { e.stopPropagation(); remove(v); }}
              style={{ minWidth: 44, minHeight: 44, border: 'none', background: 'rgba(255,255,255,.25)', color: '#fff', width: 16, height: 16, borderRadius: '50%', cursor: 'pointer', fontSize: 11, lineHeight: 1, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
          </span>
        ))}
