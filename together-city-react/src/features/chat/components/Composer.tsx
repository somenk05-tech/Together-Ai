...
export function Composer({ onSend, onTyping }: { onSend: (body: string) => void; onTyping: (t: boolean) => void }) {
  ...
  return (
    <form onSubmit={submit} style={{ display: 'flex', gap: 10, padding: 14, borderTop: '1px solid var(--line)', flexShrink: 0, background: 'var(--card, #fff)', paddingBottom: 'calc(14px + var(--safe-bottom, 0px))' }}>
