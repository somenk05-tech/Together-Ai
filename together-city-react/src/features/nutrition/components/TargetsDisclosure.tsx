 ...
 * anyone.
 *
 * IT IS A `Fold` SINCE 8 SEP. It was the app's fourth disclosure implementation
 * — its own `aria-expanded` with no `aria-controls`, a `▸` that rotated, and a
 * grid-template-rows animation nothing else in the city has. The fold list in
 * a-read-section-folds-itself.test.ts had already named it as the one entry
 * that should become a Fold; this is that. It wears `.fold-inline`, because a
 * disclosure hanging off a number is not a chapter and must not wear a card.
 */
export function TargetsDisclosure({ p }: { p: Prescription }) {
  ...
  return (
    <div style={{ marginTop: 10 }}>
      <Fold title="How we calculated this" face="fold-inline" panel="fold-inline-open">
        <div style={{ fontSize: 'var(--fs-2)', fontWeight: 800, letterSpacing: '.05em', textTransform: 'uppercase', color: 'var(--muted)' }}>
          {equation}
        </div>
        ...
        <div style={{ marginTop: 10 }}>
          {steps.map((s, i) => (
            <div key={s.label} style={{
              ...
              <span style={{ flex: 1, minWidth: 0, fontSize: 'var(--fs-3)' }}>{s.label}</span>
              <span style={{ fontSize: 'var(--fs-3)', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>{s.value}</span>
            </div>
          ))}
        </div>
        ...
        {notes.map((n) => (
          <p key={n} className="muted" style={{ fontSize: 'var(--fs-2)', lineHeight: 1.55, margin: '9px 0 0' }}>{n}</p>
        ))}

        <div style={{ marginTop: 11, paddingTop: 9, borderTop: '1px solid var(--line)' }}>
          <div className="muted" style={{ fontSize: 'var(--fs-1)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.04em' }}>
            What we used
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 14px', marginTop: 5 }}>
            {Object.entries(inputs).map(([k, v]) => (
              <span key={k} className="muted" style={{ fontSize: 'var(--fs-2)' }}>{humanKey(k)}: <b style={{ color: 'var(--ink)' }}>{String(v)}</b></span>
            ))}
          </div>
        </div>
      </Fold>
