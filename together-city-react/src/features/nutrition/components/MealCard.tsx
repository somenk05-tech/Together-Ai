  ...
  return (
    <div className="card lift" style={{ padding: 0, overflow: 'hidden', borderRadius: 16, opacity: skipped ? 0.55 : 1 }}>
      <div style={{ position: 'relative', aspectRatio: '16 / 10', display: 'flex', alignItems: 'flex-end', background: `linear-gradient(140deg,${color}22,${color}44)` }}>
        <span style={{ position: 'absolute', top: 10, left: 12, background: 'rgba(20,20,18,.72)', color: '#fff', fontSize: 11, fontWeight: 700, padding: '2px 9px', borderRadius: 999 }}>🍽️ {portionLabel}</span>
      ...
      <div style={{ padding: '14px 16px 16px' }}>
        ...
        <span className="muted" style={{ fontSize: 11.5 }}> · {n > 1 ? `${r.kcal} kcal/person · ${portionLabel}` : `${grams} g/plate`}</span>
