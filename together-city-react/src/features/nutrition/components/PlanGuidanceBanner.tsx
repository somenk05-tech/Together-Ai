    ...
    >
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginBottom: 8 }}>
        ...
        {guidance.modes.map((m) => (
          <span
            key={m.key}
            style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.04em', color: 'var(--accent-ink)', background: 'var(--card)', border: '1px solid var(--accent)', borderRadius: 'var(--r-full)', padding: '2px 12px' }}
          >
            {m.label}
          </span>
        ))}
