...
export function FamilySnacks({ recipes, family, dayIndex }: { recipes: Recipe[]; family: FamilyState; dayIndex: number }) {
  ...
  return (
    <div className="card" style={{ padding: '14px 16px' }}>
      <span style={{ fontSize: 9.5, fontWeight: 800, letterSpacing: '.04em', textTransform: 'uppercase', color: 'var(--accent-ink)', background: 'var(--accent-soft)', padding: '2px 8px', borderRadius: 'var(--r-full)' }}>Personal snacks</span>
        ...
        return (
          <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, padding: '8px 0', borderTop: '1px solid var(--line)' }}>
            <div style={{ minWidth: 0 }}>
              ...
              <span style={{ fontSize: 9, fontWeight: 800, textTransform: 'uppercase', color: 'var(--accent-ink)', background: 'var(--accent-soft)', padding: '1px 6px', borderRadius: 'var(--r-full)' }}>{NEED_LABEL[m.need]}</span>
