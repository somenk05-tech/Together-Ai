...
const STYLE = `
.legal-root { --lg-line:#e2ddd1; --lg-paper:#f2efe7; --lg-ink:#1d1b17; --lg-accent:#b08d3e; --lg-muted:#8a8375; }
...
.doc-body p { margin:0 0 1em; color:#33302a; }
.doc-body ul { margin:0 0 1.1em; padding-left:1.35em; }
.doc-body li { margin:0 0 .55em; color:#33302a; }
.doc-body strong { font-weight:600; color:#1d1b17; }
.doc-body .note { background:#fbf8f0; border-left:3px solid var(--lg-accent); padding:14px 18px; border-radius:0 10px 10px 0; margin:0 0 1.2em; font-size:.96em; color:#5a4a1f; }
.doc-body .tf { background:#fdf0d0; border-bottom:1px dashed #b08900; padding:0 4px; border-radius:3px; font-family:'IBM Plex Mono',monospace; font-size:.82em; color:#7a5c00; }
`;

function pill(drafted: boolean) {
  return drafted
    ? { label: 'Published draft', bg: '#f3ead6', fg: '#856210' }
    : { label: 'In preparation', bg: '#efeae0', fg: '#8a8375' };
}
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 20, background: 'rgba(242,239,231,.9)', backdropFilter: 'saturate(1.4) blur(10px)', borderBottom: '1px solid var(--lg-line)' }}>
        <div style={{ ...shell, padding: '15px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
          <Link to="/legal" style={{ display: 'flex', alignItems: 'center', gap: 12, color: 'var(--lg-ink)' }}>
            <span className="serif" style={{ width: 34, height: 34, borderRadius: 9, background: 'var(--lg-accent)', color: '#fff', display: 'grid', placeItems: 'center', fontWeight: 700, fontSize: 18 }}>TC</span>
...
function DraftNote() {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 11, background: '#fbf5e6', border: '1px solid #efe0b8', borderRadius: 12, padding: '14px 18px', margin: '26px 0 34px' }}>
      <span style={{ fontSize: 17 }}>⚖</span>
      <p style={{ margin: 0, fontSize: 13.5, color: '#6b5a2a' }}>
        <strong style={{ color: '#5a4a1f' }}>Draft for legal review.</strong> These documents are written to a publishable standard but are templates. They must be reviewed and finalised by an Indian technology lawyer before publication; details marked <span className="doc-body"><span className="tf">like this</span></span> need to be filled in.
      </p>
    </div>
  );
}
  ...
  return (
    <main style={{ ...shell, padding: '0 28px 90px' }}>
      ...
      <div style={{ position: 'relative', marginBottom: 40, maxWidth: 440 }}>
        <span style={{ position: 'absolute', left: 15, top: '50%', transform: 'translateY(-50%)', color: '#a49c8b', fontSize: 15 }}>⌕</span>
        <input type="text" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search policies…" aria-label="Search policies"
          style={{ width: '100%', padding: '13px 16px 13px 40px', border: '1px solid #ddd7c9', borderRadius: 11, background: '#fffefb', fontFamily: 'inherit', fontSize: 15, color: 'var(--lg-ink)', outline: 'none', boxSizing: 'border-box' }} />
      </div>
      ...
      {volumes.map((vol) => (
        <section key={vol.id} style={{ marginBottom: 46 }}>
          ...
          <p style={{ margin: '0 0 20px', color: '#7c7563', fontSize: 14.5 }}>{vol.desc}</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 14 }}>
            {vol.items.map((item) => {
              const pl = pill(item.drafted);
              return (
                <Link key={item.id} to={`/legal/policy/${item.id}`} style={{ display: 'flex', flexDirection: 'column', gap: 9, background: '#fffefb', border: '1px solid #e6e1d4', borderRadius: 14, padding: '20px 20px 18px' }}>
                  ...
                  <span style={{ fontSize: 13.5, color: '#7c7563', lineHeight: 1.5 }}>{item.short}</span>
                </Link>
              );
            })}
          </div>
      ...
      <footer style={{ borderTop: '1px solid var(--lg-line)', paddingTop: 26, marginTop: 14, color: 'var(--lg-muted)', fontSize: 13, lineHeight: 1.7 }}>
        <p style={{ margin: '0 0 4px' }}><strong style={{ color: '#57503f' }}>{CO.company}</strong> — operator of Together City ({CO.domain}).</p>
...
function PolicyView({ id }: { id: string }) {
  ...
  return (
    <main style={{ ...shell, padding: '34px 28px 90px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 11, background: '#fbf5e6', border: '1px solid #efe0b8', borderRadius: 11, padding: '13px 16px', marginBottom: 34 }}>
        <span style={{ fontSize: 15 }}>⚖</span>
        <p style={{ margin: 0, fontSize: 13, color: '#6b5a2a' }}>Template pending legal review — have Indian legal counsel finalise before publishing.</p>
      </div>
      ...
      <div className="legal-grid" style={{ display: 'grid', gridTemplateColumns: '230px 1fr', gap: 52, alignItems: 'start' }}>
        <nav style={{ position: 'sticky', top: 82, display: 'flex', flexDirection: 'column', gap: 2 }}>
          <span className="mono" style={{ fontSize: 10.5, letterSpacing: '.1em', textTransform: 'uppercase', color: '#a49c8b', marginBottom: 10 }}>On this page</span>
          {p.sections.map((s, i) => (
            <a key={i} href={`#sec${i}`} onClick={(e) => { e.preventDefault(); scrollTo(`sec${i}`); }}
              style={{ fontSize: 13, color: '#7c7563', padding: '5px 0 5px 12px', lineHeight: 1.4, borderLeft: '2px solid transparent' }}>{s.h}</a>
          ))}
        </nav>

        <article style={{ minWidth: 0, maxWidth: '70ch' }}>
          {p.tldr && p.tldr.length > 0 && (
            <div style={{ background: '#f6f1e6', border: '1px solid #e6ddc9', borderRadius: 14, padding: '22px 24px', marginBottom: 40 }}>
              ...
              <ul style={{ margin: 0, paddingLeft: 20 }}>
                {p.tldr.map((point, i) => <li key={i} style={{ margin: '0 0 8px', color: '#4a4433', fontSize: 14.5, lineHeight: 1.55 }}>{point}</li>)}
              </ul>
            </div>
          )}
          ...
          {related.length > 0 && (
            <div style={{ borderTop: '1px solid var(--lg-line)', paddingTop: 26, marginTop: 44 }}>
              <div className="mono" style={{ fontSize: 10.5, letterSpacing: '.1em', textTransform: 'uppercase', color: '#a49c8b', marginBottom: 14 }}>Related documents</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
                {related.map((r) => (
                  <Link key={r.id} to={`/legal/policy/${r.id}`} style={{ fontSize: 13, padding: '7px 13px', border: '1px solid #ddd7c9', borderRadius: 20, color: '#57503f', background: '#fffefb' }}>{r.title}</Link>
                ))}
              </div>
            </div>
          )}
