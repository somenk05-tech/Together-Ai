...
P.privacy = {
  ...
  sections: [
    ...
    { h: '7. Sharing and disclosure', html:
      ...
      ${li(['With third-party providers you transact with (sellers, local service providers, hotels, labs, creators) to fulfil your request;', 'With processors (Data Processors) who act on our instructions under contract — including Anthropic, PBC, whose Claude models perform the AI analysis behind reading and filing medical documents, skin and hair photo analysis, meal-plan narration and Mira, under commercial terms that do not permit training on your data; we send the content needed for the analysis and not your name or account;', 'With payment partners and, where required, regulators and law-enforcement under a valid legal request;', 'In a business transfer, subject to this Policy.'])}
...
P.healthdata = {
  ...
  tldr: [
    ...
    'No other hub reads your blood markers until you say so. The switch is off by default and is asked for, once, on the Record Analysis page.',
    'Reading and filing a document is done by an AI model run by Anthropic, on our instructions, with your name left out of the request.',
  ...
  sections: [
    ...
    { h: '3. Who reads it inside the city', html: `<p>By default, <strong>nobody but you</strong>. The Nutrition, Beauty and Fitness hubs can build better plans from your blood markers, and they ask first: the Record Analysis page puts the question once, with <em>Turn it on</em> and <em>Keep it off</em>, and reads nothing until you choose. You can change the answer at any time on the same page, and the change is immediate. A hub that is off reads no marker; there is no partial sharing.</p>` },
    { h: '4. The AI that reads a report', html: `<p>A document you upload to Health Records — a report, prescription or scan, as an image or PDF — is read by a large language model provided by Anthropic PBC (Claude), acting as our Data Processor under contract, to work out what kind of document it is and file it, to read the name printed on it so we can check it against the name on your account — a report in somebody else's name is refused and deleted, and one spelled differently waits for you to confirm it — to extract the markers from a blood report, and to write a plain-language interpretation. On Record Analysis the same processor reads what was taken from all of your documents together to write one overview of your history. The request carries the report and your markers and does not carry your name; the greeting is added by us afterwards. Anthropic does not use our requests to train its models under the terms we hold with it. The same processor reads beauty photographs when you ask for an assessment, and the Beauty page says so beside the button.</p>` },
