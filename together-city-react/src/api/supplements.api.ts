...
/* THE MULTIVITAMIN ASSESSMENT was read here until 9 Sep, when the owner
 * removed the page it fed. The wire is still there — GET /fitness/
 * multivitamins answers, and the engine and its specs are untouched — but
 * nothing in the city asks it, so the schema and the hook came out with the
 * screen rather than sitting in the bundle waiting for a caller. */
