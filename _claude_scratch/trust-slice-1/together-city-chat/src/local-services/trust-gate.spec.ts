import { allowanceLeft, dayStartUtc, releasable, shouldHold, waitingSentence } from './trust-gate';
...
describe('what the owner reads', () => {
  it('counts people, not policy', () => {
    expect(waitingSentence(0)).toContain('5 new neighbours a day');
    expect(waitingSentence(1)).toBe('1 neighbour is waiting to reach you.');
    expect(waitingSentence(3)).toBe('3 neighbours are waiting to reach you.');
    // Never phrased as an allowance spent — one of these gets a form filled in.
    expect(waitingSentence(3)).not.toMatch(/limit|used|quota|remaining/i);
  });
});
