...
import { useListingTrust, useSubmitVerification, type DocKind, type EntityKind, type ListingTrust, type TrustTier } from './api';
 ...
 */
export function TrustBadge({ trust }: { trust: { tier: TrustTier; label: string | null; blurb: string | null } | null | undefined }) {
...
export function TrustNote({ trust }: { trust: { label: string | null; blurb: string | null } | null | undefined }) {
  if (!trust?.blurb) return null;
  return <p className="muted" style={{ fontSize: 11.5, margin: 0 }}>{trust.blurb}</p>;
}

const ENTITY_ORDER: EntityKind[] = ['individual', 'proprietor', 'registered', 'company'];
...
export function VerificationTab({ listingId }: { listingId: string }) {
  ...
  const kind = (entityKind || t.entityKind || '') as EntityKind | '';
  const chosenDoc = (docKind || t.docKind || '') as DocKind | '';
          ...
          <div style={{ display: 'grid', gap: 8, marginTop: 12 }}>
            <label style={{ fontSize: 12 }}>
              ...
              <select style={field} value={kind} onChange={(e) => setEntityKind(e.target.value as EntityKind)}>
                ...
                {ENTITY_ORDER.map((k) => <option key={k} value={k}>{ENTITY_LABEL[k]}</option>)}
              </select>
            </label>
