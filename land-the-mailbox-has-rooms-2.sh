#!/usr/bin/env bash
# land-the-mailbox-has-rooms-2.sh  ·  run from the REPO ROOT
#
# Project folders in Together City Mail.
#
# WHY THERE IS A -2. The first script reached the web app's vitest and stopped
# there, before the commit, so nothing landed. One test failed:
# one-bag.test.ts holds every hub's rail to a contiguous 01…0N and the new
# Projects entry was numbered '00' — a nice idea about hierarchy that the rail
# does not have, since '00' is only meaningful next to '01' if you already know
# which one is the mailbox. The rail is renumbered 01…08 and Projects is simply
# first, which is what it is. That is the ONLY difference from the first
# script: one file, eight index strings.
#
# EVERYTHING THE FIRST RUN PROVED STILL HOLDS, and the API half is now proved
# rather than asserted: prisma validate, prisma generate, api tsc, the mail
# jest suites, lint at its 127 baseline and the api build all passed natively
# on that run. The Prisma client the bridge could not regenerate was the only
# thing standing between "written" and "type-checked", and it is checked now.
#
...
# its own search, its own Compose — drawn with the furniture Mail already owns.
#
# WHAT FILLS A ROOM: you composed from inside it, the trail is already filed
# and this message inherited it, or you moved it in by hand. Nothing guesses.
# No sender lists, no domain matching, no subject scoring — so no message can
# land in a room nobody named, and a new project opens EMPTY with its empty
# state as the instruction.
#
...
# the one file that changed since -1: the rail renumbered 01…08
verify 77fbcc2a881f92f6d6116bb0fd2e46685d7f7c70ee760d65d44bf3f164224a5f "$W/src/config/hubs.ts"
...
# type error the bridge reported was that and nothing else.
...
git add $A/prisma/schema.prisma \
        ...
        $W/src/index.css \
        land-the-mailbox-has-rooms-2.sh
...
Projects takes 01 in the rail and everything after it moves down one. It was
briefly '00', which one-bag.test.ts caught: a rail is a numbered list and a
zeroth item is a claim about hierarchy the rail cannot draw. Being first says
the same thing and needs no explaining.
